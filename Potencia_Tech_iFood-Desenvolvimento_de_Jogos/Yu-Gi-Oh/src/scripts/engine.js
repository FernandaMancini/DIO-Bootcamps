...
async function setCardsField(cardId){
   await removeAllCardsImages();
    let computerCardIt = await getRandomCardId();
   
    await ShowHiddenCardsFieldsImages(true);

    await hiddenCardDetails(); 

    await drawCardsInField(cardId, computerCardIt)
    
    let duelResults = await checkDuelResults(cardId, computerCardIt)
    await updateScore();
    await drawButton(duelResults);
}

async function drawCardsInField(cardId, computerCardIt){
    state.fieldCards.player.src = cardData[cardId].img;
    state.fieldCards.computer.src = cardData[computerCardIt].img;
}
